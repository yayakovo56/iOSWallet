...
//分页控件
@property (nonatomic, strong) CCWPageControl *pageControl;
//当前显示的imageView
...
- (CCWPageControl *)pageControl {
    if (!_pageControl) {
        _pageControl = [[CCWPageControl alloc] init];
...
- (void)setImageCornerRadius:(CGFloat)imageCornerRadius
{
    ...
    _currImageView.layer.cornerRadius = imageCornerRadius;
    _otherImageView.clipsToBounds = YES;
    _otherImageView.layer.cornerRadius = imageCornerRadius;
}
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    if (!_pageImageSize.width) {// 没有设置图片，系统原有样式
        ...
        size = CGSizeMake(_pageImageSize.width * self.pageControl.numberOfPages + 4 * (self.pageControl.numberOfPages - 1), _pageImageSize.height);
    }
    ...
    }else if (_pagePosition == PositionBottomOut){ // By SYLing
        self.pageControl.center = CGPointMake(self.width * 0.5, centerY + 24);
    }else{
...
- (void)startTimer {
    ...
    self.timer = [NSTimer ccw_timerWithTimeInterval:_time < 1? DEFAULTTIME: _time repeats:YES block:^(NSTimer *timer) {
        [weakSelf nextPage];
    }];
