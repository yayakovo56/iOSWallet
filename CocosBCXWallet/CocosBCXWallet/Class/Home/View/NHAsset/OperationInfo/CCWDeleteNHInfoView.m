...
@implementation CCWDeleteNHInfoView
...
{
    if (!_tipLabel) {
        _tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(self.tableView.frame), CCWScreenW - 30, 14)];
        _tipLabel.text = CCWLocalizable(@"注意：资产删除后不可恢复");
        _tipLabel.textColor = [UIColor getColor:@"F56151"];
        _tipLabel.font = CCWFont(12);
        [self.containerView addSubview:_tipLabel];
    }
    return _tipLabel;
}
...
{
    if (!_nextBtn) {
        _nextBtn = [[UIButton alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(self.tableView.frame) + 27, CCWScreenW - 30, 50)];
