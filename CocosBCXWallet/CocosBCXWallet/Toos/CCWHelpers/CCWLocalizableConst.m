//
//  CCWLocalizableConst.m
//  CocosBCXWallet
//
//  Created by 邵银岭 on 2019/1/29.
//  Copyright © 2019年 邵银岭. All rights reserved.
//

#import "CCWLocalizableConst.h"

#pragma mark - 用户语言
NSString * const CCWUserLanguage = @"userLanguage";//用户语言
NSString * const CCWlproj = @"lproj";

NSString * const CCWLanzh = @"zh-Hans";//中文文件
NSString * const CCWLanEn = @"en";//英文文件

NSString * const CCWLanzhHans = @"zh-Hans-CN";//简体
NSString * const CCWLanEn_us = @"en-US";//美文
NSString * const CCWLanBase = @"Base";// 默认语言
//NSString * const CCWLanzhHant = @"zh-Hant";//繁体
//NSString * const CCWLanzh_CCW = @"zh-CCW";//台湾
//NSString * const CCWLanzh_HK = @"zh-HK";//香港

